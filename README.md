\# 🛒 Zepto Inventory \& Pricing Analysis using SQL



SQL Data Analysis Project using Zepto Inventory Dataset | Inventory, Pricing \& Business KPI Analysis



!\[Project Output](screenshots/Screenshot%202026-06-14%20151329.png)



\## Project Overview



This project is a complete SQL-based data analysis case study built using a real-world Zepto product inventory dataset. The objective was to simulate how a Data Analyst works with retail and e-commerce data to uncover business insights related to pricing, inventory management, discount strategies, and product performance.



The project covers the complete analytics workflow:



Data Exploration



Data Cleaning



Business KPI Analysis



Inventory \& Pricing Insights



SQL-Based Decision Making



This project was developed using \*\*MySQL\*\* and focuses on solving practical business problems through SQL queries.



\---



\## Dataset Overview



The dataset contains 3,000+ product records (SKUs) from Zepto across multiple categories including:



\* Fruits \& Vegetables

\* Dairy \& Breakfast

\* Snacks \& Munchies

\* Beverages

\* Packaged Foods

\* Personal Care

\* Household Essentials



Each row represents a unique product with information related to pricing, discounts, stock availability, quantity, and weight.



\### Dataset Columns



\* Category

\* name

\* mrp

\* discount\_percent

\* available\_quantity

\* discounted\_selling\_price

\* weight\_in\_gms

\* out\_of\_stock

\* quantity



\---



\## Data Exploration



The first phase focused on understanding the dataset structure and identifying potential data quality issues.



\### Tasks Performed



\* Checked total number of records

\* Inspected dataset structure

\* Identified missing values

\* Explored unique product categories

\* Analyzed stock availability

\* Verified pricing consistency

\* Checked for duplicate records



\---



\## Data Cleaning



To ensure accurate analysis, several data-cleaning steps were performed:



\### Cleaning Activities



\* Renamed columns using snake\_case convention

\* Validated missing values

\* Checked duplicate records

\* Standardized column names

\* Verified pricing and inventory fields

\* Removed inconsistencies where necessary



\---



\## Business KPI Analysis



\### Q1. Find the Top 10 Best-Value Products Based on Discount Percentage



\### Q2. Identify Products with High MRP but Currently Out of Stock



\### Q3. Calculate Estimated Revenue for Each Category



\### Q4. Find All Products Where MRP is Greater Than ₹500 and Discount is Less Than 10%



\### Q5. Identify the Top 5 Categories Offering the Highest Average Discount Percentage



\### Q6. Find the Price per Gram for Products Weighing More Than 100g and Rank Them by Best Value



\### Q7. Categorize Products into Inventory Segments (Low, Medium, Bulk)



\### Q8. Calculate Total Inventory Weight for Each Category



\### Q9. Identify Categories with the Highest Stock-Out Risk



\### Q10. Estimate Potential Revenue Loss Due to Out-of-Stock Products



\### Q11. Identify Dead Inventory (High Stock, Low Discount Products)



\### Q12. Calculate Discount Leakage by Category



\### Q13. Determine Category Dominance by Inventory Value Contribution



\### Q14. Identify Products Suitable for Portfolio Optimization or Discontinuation



\---



\## Key Business Insights



\* Identified categories offering the highest discounts.

\* Detected products with high inventory risk.

\* Estimated category-level revenue potential.

\* Highlighted products providing the best value for money.

\* Evaluated discount leakage across categories.

\* Identified potential dead inventory.

\* Analyzed inventory concentration by category.

\* Recommended products for portfolio optimization.



\---



\## Tools Used



\* MySQL

\* MySQL Workbench

\* SQL



\### SQL Concepts Applied



\* Aggregations

\* GROUP BY

\* ORDER BY

\* CASE Statements

\* Subqueries

\* Common Business KPIs

\* Data Cleaning Techniques

\* Inventory Analysis

\* Pricing Analysis



\---



\## Project Structure



\* Data Import

\* Data Exploration

\* Data Cleaning

\* Business KPI Analysis

\* Inventory \& Pricing Insights



\---



\## Conclusion



This project demonstrates how SQL can be used to transform raw inventory data into actionable business insights. By analyzing product pricing, discounts, inventory levels, and category performance, the project showcases practical data analyst skills commonly used in retail and e-commerce environments.



The project helped strengthen skills in SQL querying, data cleaning, business analysis, and problem-solving using real-world data.

